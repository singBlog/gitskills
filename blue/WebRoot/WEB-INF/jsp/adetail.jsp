  	...
  	<div class="content">
		<div id="main">
   			<div id="detial">
				...
				<br>
   				<p class="date">类别：<a href="showArticleList.htm?type=${AD.articleType.id}" title="查看该分类中所有文章">${AD.articleType.typeName }</a> 
   				 <s:if test="#log!=null">
   				   <s:if test="#log.username == #AD.author">
   				 |  <a href="ArticleEdit.htm?id=${AD.id}">编辑</a>  | <a href="javascript:delArticle(${AD.id})">删除</a>
   				   </s:if>
   				 </s:if>
   				  | <a href="#comment">评论(${fn:length(AD.articleComments)})</a> | <a href="javascript:void(0)">浏览(${AD.scan})</a></p>
   				<a name="comment"></a>
	   			...
	   			<div id="dcomment">
	   				<s:iterator id="COM" value="#request.ARTICLE_COMMENT" status="com">
		   				<div class="info">
		   					...
		   					<s:if test="#log!=null"><div class="op"><a href="#reply" onclick="reply(1,'${COM.id}')">回复</a> 
		   					
		   					 <s:if test="#log.username == #AD.author">
		   					| <a href="javascript:delComment(${COM.id})">删除</a>
		   					</s:if>
