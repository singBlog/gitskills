...
function showProfile(){
	g_pop=new Popup({ contentType:1,isReloadOnClose:true,width:'400',height:'490'});
...
/********************/
var realname,password,confpassword,birthplace,liveplace,email,introduce,blogName,blogSign;
function getProfileValue(){
	...
	blogSign=$("#blogSign").val();
}

var r,p,cp,bp,lp,e,it,bname,bsign;
...
function editProfile(){
	...
	$.getJSON("editUserInfo.htm",{uid:id,realName:r,password:p,confPassword:cp,birthPlace:bp,livePlace:lp,email:e,introduce:it,blogName:bname,blogSign:bsign},
		function(data){
			if(data.tips=="no_login"){
				login();
			}
				...
				//alert('修改个人资料成功');
				toalert();
				//showError("修改个人资料成功,请重新登录");
			    //login();
			    
			//
		//	window.location.href=window.parent.location.href;
			//$('#dialogBox').hide();
			//window.parent.document.body.removeChild(window.parent.document.getElementByIdx_x_x_x("dialogCase"))
			//
...
function toalert(){
	...
	this.parent.Popup.prototype.reset_m(); //刷新父页面
...
function regUserInfo(){
	...
	$.getJSON("regUserInfo.htm",{username:username,realName:r,password:p,confPassword:cp,birthPlace:bp,livePlace:lp,email:e,introduce:it,blogName:bname,blogSign:bsign},
		function(data){
			if(data.tips=="101010"){
				showError("用户名已经存在！");
			}
			else if(data.tips=="000000")
				//alert('修改个人资料成功');
				toalert();
				//showError("修改个人资料成功,请重新登录");
			    //login();
