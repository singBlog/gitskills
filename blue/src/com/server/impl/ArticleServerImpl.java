...
public class ArticleServerImpl implements ArticleServer {
	...
	public List getAllArticleType(int userid) {
		return articleDao.findAllType(userid);
	}
	
	public int getAllArticleNum(int userid){
		return articleDao.findAllArticle(userid).size();
	}
	...
	public List getLatestArticleInEachType() {
		...
		//TODO
		List type=this.getAllArticleType(000);
	...
	public int addNewArticleType(String articleTypeName,String articleDesc, int userid) {
		ArticleType at=new ArticleType();
		if(articleDao.getArticleTypeByName(articleTypeName,userid)!=null){
			return -1;
		}else{
			at.setTypeName(articleTypeName);
			at.setTypeDesc(articleDesc);
	...
	public List getArticleByPage(int pageNo,int pageSize,int type,int userid) {
		return articleDao.findArticleByPage(pageNo,pageSize,type,userid);
	}
	...
	public List getLatestArticle(int top,int userid) {
		return articleDao.findLatestArticle(top,userid);
