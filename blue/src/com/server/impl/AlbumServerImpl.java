...
public class AlbumServerImpl extends MyHibernateDaoSupport implements AlbumServer {
	...
	public int addNewImgType(String typeName,String typeDesc, int userid) {
	...
	public List getAllImgType(int userid) {
		return albumDao.findAllImgType(userid);
	}
	...
	public List getOldestImgInEachType(int top, int userid) {
		...
		List type;
		if(top>0)
			type=albumDao.findLatestAlbum(top,userid);
		else
			type=this.getAllImgType(userid);
		List oldestList=new ArrayList();
		for(int i=0;i<type.size();i++){
			Integer type_id=((ImgType)type.get(i)).getId();
			List ilist=albumDao.findImgByImgTypeId(type_id);
	...
	public List getLatestImgByTypeInTop(int type, int top,int userid) {
		return albumDao.findLatestImg(type, top,userid);
	}
