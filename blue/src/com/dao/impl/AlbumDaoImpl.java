...
public class AlbumDaoImpl extends MyHibernateDaoSupport implements AlbumDao {
	...
	@SuppressWarnings("unchecked")
	public List findAllImgType(int userid) {
		return super.getHibernateTemplate().find("from ImgType where userid = " + userid);
	}
	@SuppressWarnings("unchecked")
	...
	@SuppressWarnings("unchecked")
	public List findLatestImg(int type, int top, int userid) {
		String hql;
		if(type==-1){
			hql="from Img where userid = ? order by time desc";
		}else{
			hql="from Img i where i.imgType.id="+type+" and i.userid = ? order by time desc";
		}
		return super.findByTop(hql, top,userid);
	}
	@SuppressWarnings("unchecked")
	public List findLatestAlbum(int top, int userid) {
		String hql="from ImgType where userid = ? order by id desc";
		return super.findByTop(hql, top,userid);
	}
	@SuppressWarnings("unchecked")
