...
public class ArticleDaoImpl extends MyHibernateDaoSupport implements ArticleDao {
	...
	public ArticleType getArticleTypeByName(String name, int userid) {
		String sql="from ArticleType at where at.typeName=? and at.userid = " +userid;
		@SuppressWarnings("unchecked")
		List list=super.getHibernateTemplate().find(sql,name);
		if(list.size()>=1){
			return (ArticleType)list.get(0);
	...
	public List findAllType(int userid) {
		...
		if (userid != 000) {
			return super.getHibernateTemplate().find("from ArticleType where userid = " + userid);	
		}
		else {
	...
	public List findArticleByTypeId(Integer type_id,int userid) {
		String hql="from Article a where a.articleType.id=? and a.userid= ? order by time desc";
		Object[] params={type_id,userid};
		return super.getHibernateTemplate().find(hql,params);
	}
	@SuppressWarnings("unchecked")
	public List findAllArticle(int userid) {
		return super.getHibernateTemplate().find("from Article where userid = " + userid);
	}
	@SuppressWarnings("unchecked")
	...
	public List findArticleByPage(int pageNo, int pageSize,int type,int userid) {
		...
		String hql;
		if(type==0)
			hql="from Article where userid = "+userid+" order by time desc";
		else
			hql="from Article a where a.articleType.id="+type+" and a.userid = "+userid+" order by time desc";
		return super.findByPage(hql, offset, pageSize);
	}
	@SuppressWarnings("unchecked")
	public List findLatestComment(int top) {
		...
		//TODO
		return super.findByTop(hql, top,null);
	}
	@SuppressWarnings("unchecked")
	public List findLatestArticle(int top,int userid) {
		String hql="from Article where userid = ? order by time desc";
		
		return super.findByTop(hql, top, userid);
	}
	@SuppressWarnings("unchecked")
