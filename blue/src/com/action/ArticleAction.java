...
/*********function************************************************/
	...
	public String addNewArticle() throws Exception {
		if(!public_validation()){
			...
			Login login = (Login) ActionContext.getContext().getSession().get("LOGIN");
			
			articleServer.addNewArticle(title, author, content, articleType, login.getId());
	...
	public String showArticleList() throws Exception{
		...
		Login login = (Login) request.getSession().get("LOGIN");
		
		if (login == null) {
			request.put("PAGE_LINK",null);
			request.put("ARTICLE_LIST", null);
			request.getSession().put("ALL_ARTICLE_TYPE",null);
			request.put("LATEST_COMMENT",null);
		}
			...
			if(type==0)
				//总个数
				totalSize=articleServer.getAllArticleNum(login.getId());
			else{
				
				//list_type=articleServer.getArticleByTypeId(type);
				list_type=articleServer.getArticleByTypeId(type,login.getId());
			...
			List alist=articleServer.getArticleByPage(showPage, pageSize,type,login.getId());
			
			request.put("PAGE_LINK",DiviedPage.getPageLink(totalPage,showPage,type,url));
			request.put("ARTICLE_LIST", alist);
			request.getSession().put("ALL_ARTICLE_TYPE",articleServer.getAllArticleType(login.getId()));
...
/*******validation*********************************************/	
	...
	public void validateAddNewArticleType() {
		...
		myid=articleServer.addNewArticleType(articleTypeName,articleDesc, login.getId());
		if(myid>0){
			tips="ok";
