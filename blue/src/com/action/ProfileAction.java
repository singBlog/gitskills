...
public class ProfileAction extends ActionSupport{
	...
	public String showProfile() throws Exception {
		...
		//Login login2 = userServer.login(login.getId());
		
		act.put("USER_INFO",login);
		//act.put("USER_INFO", userServer.login(uid));
		//act.put("LATEST_ARTICLE",articleServer.getLatestArticle(article_top));
		
		act.put("LATEST_ARTICLE",articleServer.getLatestArticle(article_top,login.getId()));
		act.put("LATEST_LEAVEMSG",leavemsgServer.getLatestMsg(msg_top,login.getId()));
		
		
		
		List latest_img_top=albumServer.getLatestImgByTypeInTop(type, img_top,login.getId());
